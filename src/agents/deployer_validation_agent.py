...
class DeployerValidationAgent:
    """
    Agent responsible for validating deployers in change management processes.
    
    This agent performs the following tasks:
    1. Load verified population data
    2. Load CI/CD deployment logs
    3. Load IAM users data
    4. Validate deployers against IAM users list
    5. Check if deployers have the correct role
    6. Flag unauthorized deployers
    7. Generate a report with flagged records and reason codes
    """

    def __init__(self, data_dir="./data/input", output_data_dir="./data/output", 
             verified_population_file=None):
        
        self.name = "DeployerValidationAgent"
        ...
        self.cicd_deployment_data = None
    ...
    def load_cicd_deployment_logs(self):
        """
        Load CI/CD deployment logs data.
        """
        data, success = self.data_extractor.load_cicd_deployment_logs()
        
        if success:
            self.cicd_deployment_data = data
            return True
        return False
    ...
    def validate_deployers_with_ai(self):
        """
        Use Azure AI Foundry to validate deployers against IAM users list.
        """
        try:
            self.logger.info("Validating deployers using Azure AI Foundry")
        
            # Check if required data is loaded
            if self.change_migration_data is None or self.cicd_deployment_data is None or self.iam_users_data is None:
                self.logger.error("Required data not loaded for deployer validation")
                return False
            ...
            self.logger.error(f"Error validating deployers with AI: {str(e)}")
            import traceback
    ...
    def _prepare_data_for_validation(self):
        """
        Prepare data for deployer validation.
        """
        # Create a copy of the change migration data with only essential columns
        validation_data = self.change_migration_data[['Change_ID', 'Asset_Name']].copy()
        
        # Merge with deployment data to get deployer information
        validation_data = pd.merge(
            validation_data,
            self.cicd_deployment_data[['Linked_Change_ID', 'Asset_Name', 'Deployer_ID', 'Deployer_Name']],
            left_on=['Change_ID', 'Asset_Name'],
            right_on=['Linked_Change_ID', 'Asset_Name'],
            how='left'
        )
        ...
        # Add IAM deployer roles as a simple comma-separated string
        deployer_roles = self.iam_users_data[self.iam_users_data['IAM_Role'] == 'Deployer']
        deployer_user_ids = ','.join(deployer_roles['User_ID'].tolist())
        validation_data['IAM_Deployer_IDs'] = deployer_user_ids
    ...
    def _create_ai_prompt(self, batch_json):
        ...
        """
        return f"""
            You are validating deployers in a change management system. Here are the records to analyze:
            {batch_json}

            For each record, determine if the deployer is authorized by checking:

            For each record, follow these exact steps:
            1. Get the Deployer_ID from the record
            2. Check if Deployer_ID is in IAM_User_IDs list
                - If NOT found or Deployer_ID is null: Status="Exception", Reason_Code="No deployment record found"
                - If found: Continue to step 3
        
            3. Check if Deployer_ID is in IAM_Deployer_IDs list
                - If NOT found: Status="Exception", Reason_Code="Unauthorized Deployer"
                - If found: Status="OK", Reason_Code="Valid deployer"
            ...
            EXAMPLE OUTPUT:
            [
                {{"Change_ID": "CHG1000", "Status": "Exception", "Reason_Code": "No deployment record found"}},
                {{"Change_ID": "CHG1001", "Status": "OK", "Reason_Code": "Valid deployer"}}
            ]
    ...
    async def _call_ai_for_analysis_async(self, prompt, batch_index, batch_size, total_records):
        ...
        try:
            ...
            Your task is to analyze change management records and validate if deployers have proper authorization."""
    ...
    def _call_ai_with_thread_and_run(self, system_prompt, user_prompt, max_retries=5):
        """
        ...
        try:
            ...
            if not agents_list:
                ...
                agent = self.client.create_agent(
                    name="Deployer Validation Agent",
                    description="Validates deployers in change management processes",
                    model=self.AGENT_MODEL_DEPLOYMENT_NAME
                )
                return agent.id
            else:
    ...
    def _process_ai_results(self, all_results, original_data):
        ...
        try:
            ...
            if not all_results:
                ...
                columns_to_remove = ['IAM_User_IDs', 'IAM_Deployer_IDs', 'Linked_Change_ID']
    ...
    def save_validation_report(self):
        """
        Save the deployer validation report.
        """
        try:
            self.logger.info("Saving deployer validation report")
            ...
            output_dir = os.path.join(self.output_data_dir, "deployer_validation")
            os.makedirs(output_dir, exist_ok=True)

            # Generate filename
            filename = "deployer_validation_report.xlsx"
            output_path = os.path.join(output_dir, filename)

            # Prepare metadata for the report
            report_metadata = {
                'report_timestamp': dt.now().strftime('%Y-%m-%d %H:%M:%S'),
                'generated_by': 'DeployerValidationAgent',
            ...
            with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
                ...
                self.logger.info(f"Deployer validation report saved to {output_path}")
                return output_path
        
        except Exception as e:
            self.logger.error(f"Error saving deployer validation report: {str(e)}")
            import traceback
            self.logger.error(f"Traceback: {traceback.format_exc()}")
            return False
    ...
    def run(self):
        """
        Run the complete deployer validation workflow.
        """
        try:
            self.logger.info("Starting DeployerValidationAgent")
            ...
            # Step 3: Load IAM users data
            success = self.load_iam_users_data()
            if not success:
                self.logger.error("Failed to load IAM users data")
                return False
        
            # Step 4: Validate deployers with AI
            success = self.validate_deployers_with_ai()
            if not success:
                self.logger.error("Failed to validate deployers with AI")
                return False
            ...
            self.logger.info(f"DeployerValidationAgent completed successfully. Output file: {output_file}")
            return True
